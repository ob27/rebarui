#!/bin/bash
# Creates a fresh, empty bench/<name> scaffold for the Kanban benchmark's 5th condition —
# kanban via @rebar-ui/placement's card-kanban block, testing whether a widened schema (the
# assignee/statusTag/addPosition fields added alongside the addPosition component fix) lets the
# DSL path hit the Sprint Board spec with pure data, no renderCard escape hatch.
# Usage: ./make-kanban-dsl-scaffold.sh kanban-opinion-dsl-01
set -e
NAME="$1"
