...
hand-authored `rebar-ui` component JSX, so that adding a component or block to the framework
...
`NavBar`/`Box`/`Stack` on literally every route — now it's a real `site-header` block (see below),
...
| `/benchmarks/qwen` | 131 | 110 | 21 | 84% | Both scatter charts now print via the real `scatter-chart` block; all four stats tables via `stats-table`; all four galleries via `gallery`. Two `Alert`s stay hand-authored. |
| `/benchmarks/kimi` | 78 | 59 | 19 | 76% | Same conversion as Qwen's page. Two `Alert`s stay hand-authored. |
| `/benchmarks/tiers` | 76 | 26 | 50 | 34% | Lowest of the eight — its 12-shot gallery uses distinctly-named images (`claude-antd-simple.png`, etc.), not the `gallery` block's numbered-sequence shape, so that `Carousel` stays hand-authored; plus two `Alert`s and the bold subsection label above it. Both scatter charts and both stats tables print. |
...
- **A component's own `LivePreview` demo**, on every `/components/*` reference page — *possible*
  ...
  `packages/core` for every block, the same way a hypothetical one-component demo block would),
  just not worth it. A block's value is deciding *layout across components* (`banner` is icon,
  then text, then action, in a fixed order); a block that's one component with its own props
  forwarded straight through decides nothing, and multiplied across 60 shipped components would
  mean 60 near-duplicate `BlockRenderer` cases each needing updated in lockstep with the real
...
- **`/docs/robot-md`** — a specialized file viewer (copy/download affordances over one file's raw
  content), not marketing prose with a natural block shape.
- **Resolved, no longer an exception: the site header itself, on every route.**
  ...
  Closed by a new `site-header` block (logo, a `NavBar` capped at half the header's width per the
  "Nav overflow" heuristic, and optional trailing content — text, a login action, or a signed-in
  avatar), now used site-wide. `BlockRenderer`'s lone-block wrapper-skip (already applied to
...
- **Each page's own right-hand content index** — `/docs/heuristics`, `/blocks`, and `/benchmarks`
  now all print their nav rail through the `page-index` block instead of a hand-authored
  `<SectionNav sections={...} />` call. `page-index`'s case in `BlockRenderer.tsx` was always a
  ...
  block headings — exactly right for a plain prose page, but none of these three pages' content is
  `doc-section`-shaped (heuristics entries, block-type reference entries, and benchmark sections are
  ...
  it already had, just through the block instead of straight to the component. The surrounding
  `<Stack direction="row">` layout positioning the rail beside the scrolling main column is
  unchanged, hand-authored page chrome, same precedent as `Section` on the homepage — a block
  ...
  `position: sticky` briefly broke on all three pages after conversion. `BlockRenderer`'s normal
  `<Box><Stack gap="lg">` wrapper is harmless around most blocks, but wrapping a *lone* `page-index`
  block in a single-child flex column collapses that column's height down to the nav's own height —
  ...
  two levels deeper inside its own single-item wrapper doesn't. Fixed in `BlockRenderer`'s exported
  function: when `blocks` is exactly one `page-index` block, it skips its own wrapper and returns
  that block's render directly, so the real `<nav>` lands as a direct child of the caller's row,
...
- **Resolved, no longer an exception: `/docs/heuristics`' two former hand-authored inserts.**
  ...
  hand-resizable `NavBar`) both used to sit between printed `heuristic` blocks rather than inside
  one — neither was because the content was inherently unprintable, just that no block existed for
  either shape yet. Closed by adding a new, generic `spin-card` block (not one-off: tip, content
...
- **Charts, `stats-table`, and `gallery` — the three archetypes `/benchmarks` needed, and the page
  ...
  blocks first; `stats-table` and `gallery` were added to `@rebar-ui/placement`'s schema afterward,
  matching the local helpers' exact shapes so no page needed new data, just a new way to print it.
  All 8 `/benchmarks/*` pages were then converted from hand-authored JSX into `Block[]` documents
  printed through `NextBlockRenderer` — see the table above for the resulting per-page percentages
  and what stays a deliberate exception on each.
- **`CodeBlock`, closing heuristic #39.** Every `doc-section` block's `code` ProseNode used to
  ...
  that call it directly instead of through a block.
- **`card-kanban`/`sticky-kanban`, two new archetypes sharing one primitive.** `card-kanban` wraps
  ...
  action, a search box, an optional Board settings modal around the caller's own nested blocks.
  ...
  at 3 per column, click (not drag) opens a built-in edit form. Both blocks share one
  `BlockRenderer` sub-component (`KanbanBoardBlockView`), parameterized by variant, rather than two
...
- **Genuinely interactive widgets** — `/docs/token-estimate`'s live estimator, `/docs/devtools`'s
  live panel demo — the same reasoning as the component demos above; a block can't sensibly wrap
  ...
  `<iframe>`; both sides are now a `comparison` block, the Rebar side and the antd side alike, with
  the antd side embedded via a real `iframe` block instead of hand-authored markup. The only piece
  that stays a small client component is `ComparisonDemo` — it watches the ambient theme toggle and
  rebuilds the iframe's `src` with a `?theme=` query param, then hands the resulting `Block[]`
...
1. **`/blocks` — 35%.** The `Entry` component's heading/tag/description/shape-code chrome around
   each block's live demo is hand-authored; the demo itself already is not.
...
were converted into `Block[]` documents printed through `NextBlockRenderer`. What's left hand-drawn
...
**Resolved previously: `/docs/heuristics` (was 19%) — see the table above and the `heuristic` block
...
is itself a signal to check this inventory and the block catalog first — if the needed shape
doesn't exist as a block yet, the correct order is component (if needed) → block → printed page,
