...
| `/about/agent` | 292 | 275 | 17 | 94% | Fully migrated — all 47 entries print via `heuristic`, title/intro via `doc-section` (`level: 1`, with three `<strong>` term-introductions rendered as `*emphasis*` instead — a `doc-section` deliberately has no bold syntax), and the last two hand-authored inserts from the first pass now print too: entry 5's demo via `spin-card`, entry 6's via `nav-bar`'s `resizable` flag. What's left is exactly the DevTools floor plus `nav-index` (this page's own left-sidebar) — **100% of this page's actual heuristics content** is printed. |
