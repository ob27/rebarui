---
title: Rebar UI — Constructs (Global / Web / Mobile)
status: living document
---

# Constructs: Global / Web / Mobile

The placement-layer's construct catalog (`packages/placement/src/schema.ts`) grew to 39 types with no
categorization beyond the topical groupings already on [`/constructs`](../apps/docs/src/app/constructs/page.tsx)
(Navigation, Feedback & content, Marketing, etc.) — useful for *browsing by content type*, but not
for the separate, real question this document answers: **which target(s) is a given construct actually
built for?** A construct reaching for a wide viewport and desktop interaction patterns isn't wrong to
...
`/components`' own filter. This document is the construct-level equivalent of that categorization,
plus (for **Global** constructs specifically) real reference values pulled from Ant Design's own
...
### Reference values for Global constructs (sourced from Ant Design's own spec pages)
...
starting-point reference the next visual pass (migration, or a real Global-construct redesign) should
...
## Mobile constructs (2 — `error-construct`, `footer`; was 0)

**`error-construct` (wraps `ErrorBlock`) shipped** — this project's first genuinely Mobile-only construct,
and the same thing the separately-tracked "dedicated failure-state constructs" backlog item was asking
for (that item and the "Mobile-construct gap" named here converged on one real piece of work, not two).
...
`banner`/`header`/`callout` already render theirs; `fullPage` defaults to `true` here (the construct's
whole reason to exist is the whole-page case — an inline, one-card failure state is small enough to
just use the real `ErrorBlock` component directly, no construct needed). No `icon` override field in the
schema, unlike the real component's own `icon` prop: an arbitrary icon isn't serializable `Construct[]`
...
`onLinkClick`/`onChipClick` in the schema (not serializable `Construct[]` data) — `links` render as real
`<a href>`s via `renderLink`, same as every other link-bearing construct.
...
landed as a `packages/core` **component** only, not yet promoted into a construct — this is still the
actual, honest state of "Mobile Constructs": two, not a rounding error either way, matching this
project's own component/construct distinction (`robot.md`'s own worked `GoalTracker` → `TodoItem` +
`goal-tracker` example: a component becomes a construct once there's a real, repeatable page-content
shape to compose it into).

**Concrete candidate for the next pass** (named, not designed — no schema/renderer work done here):
- A **numeric-entry/checkout-flow construct** composing `Selector`/`NumberKeyboard`/`PinInput` — named
  as a plausible shape, not scoped further.

## The still-separately-tracked items this document doesn't resolve

Two items from the original constructs-documentation-restructuring backlog entry are explicitly **not**
covered here — they were named alongside the Global/Web/Mobile split but are their own separate,
unscoped pieces of work:

- **The "research exception" page pattern** — noted as a Global-construct candidate in the original
  backlog entry, not designed here.
- **Dedicated failure-state constructs** (see the Mobile-constructs gap above) — distinct from the
...
construct for a mobile vs. a web page — see `packages/core/robot.md`'s "Building constructs" section, whose
construct catalog now names each construct's category inline, and `/constructs`' own page, which still groups by
