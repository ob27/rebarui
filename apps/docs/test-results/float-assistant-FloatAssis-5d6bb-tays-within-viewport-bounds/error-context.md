...
- Name: float-assistant.spec.ts >> FloatAssistant >> panel stays within viewport bounds
- Location: tests/float-assistant.spec.ts:74:7
...
```ts
  ...
  2   | 
  3   | test.describe("FloatAssistant", () => {
  4   |   test("renders without canvas color errors", async ({ page }) => {
