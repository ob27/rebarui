...
/**
 * Serves the real Agents.md as a download, straight from packages/core — the same file that ships
 * in the rebar-ui npm tarball — rather than a duplicated static copy under apps/docs/public. Its
 * own route segment can't literally be named "Agents.md" (Next.js route segments aren't arbitrary
 * filenames), so the download filename is set explicitly via Content-Disposition instead.
 */
export function GET() {
  const filePath = path.join(process.cwd(), "..", "..", "packages", "core", "agents.md");
