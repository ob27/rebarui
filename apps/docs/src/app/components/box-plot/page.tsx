...
const BLOCKS: Block[] = [
  ...
  { type: "props-table", heading: "Props", rows: componentProps["BoxPlot"] ?? [] },
  {
    ...
    body: [
      {
        kind: "text",
        text: "A statistical distribution chart — each group drawn as a vertical box-and-whisker from a pre-computed five-number summary (min/q1/median/q3/max), side by side on a shared numeric y-scale. Built for comparing the spread of several groups' worth of a measured value at a glance, without plotting every raw sample. This component draws the box/whiskers from the summary you supply — it doesn't compute quartiles from raw sample data. `title` renders as a real, visible caption per [Design Heuristics](/docs/heuristics) #16 (charts ship with context, not just an accessible name); colors default to a small built-in palette, cycled by group index, when a group omits its own.",
      },
    ],
  },
  {
