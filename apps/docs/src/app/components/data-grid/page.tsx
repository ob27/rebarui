...
const BLOCKS: Block[] = [
  ...
  { type: "props-table", heading: "Props", rows: componentProps["DataGrid"] ?? [] },
  {
    ...
    body: [
      ...
      {
        kind: "text",
        text: '`groupBy` only reorganizes rows into sections — it never aggregates, and every original row still renders on its own underneath its group header. For a grid where each cell is instead a real `sum`/`count`/`average` collapsing many rows into one, see [PivotTable](/components/pivot-table).',
      },
    ],
  },
  {
