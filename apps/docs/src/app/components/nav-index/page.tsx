...
const BLOCKS: Block[] = [
  {
    ...
    body: [
      {
        kind: "code",
        code: '<NavIndex items={[{ label: "Avatar", href: "/components/avatar", category: "web" }, { label: "Action Sheet", href: "/components/planned/action-sheet", category: "mobile", status: "Planned" }]} categoryLabels={{ web: "Web", mobile: "Mobile" }} />',
      },
    ],
  },
    ...
    body: [
      {
        kind: "list",
        items: [
          "An un-categorized item (no `category` field) always renders above the filterable list, in bold, untouched by search or category state — a real navigation link, not the same thing as the \"All\" category chip beside it, even when the two are worded almost identically (see [Design Heuristics](/docs/heuristics#storage-context) heuristic #4).",
          'A `status` on any item ("Planned", "Deprecated") renders as a real `Tag` pill next to its label — never appended into the label string itself. See [Design Heuristics](/docs/heuristics) heuristic #41: a status is metadata about the item, not part of its name.',
        ],
      },
      {
        kind: "text",
        text: "Search and category chips only appear once the list passes 12 items — a short list doesn't need filter chrome, and showing one anyway is chrome with nothing to justify it (see [Design Heuristics](/docs/heuristics#ia-pyramid), heuristic #11). Try searching or filtering the live example below.",
      },
    ],
  },
  {
  ...
  return (
    <Stack gap="lg">
      ...
      <Text color="secondary">
        ...
        got hand-rolled independently twice on this site&apos;s own <code>/components</code> and{" "}
