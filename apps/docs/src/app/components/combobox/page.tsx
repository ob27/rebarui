...
const BLOCKS: Block[] = [
  ...
  { type: "props-table", heading: "Props", rows: componentProps["Combobox"] ?? [] },
  {
    ...
    body: [
      {
        kind: "text",
        text: "Checked directly against source: `Select` has no type-to-filter today — it's browsing-only. `Combobox` is the type-to-filter half (see [Design Heuristics](/docs/heuristics) heuristic #27, multiple input methods) — a text input that filters its own option list as the user types.",
      },
    ],
  },
  {
