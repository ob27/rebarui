/**
 * Tier per block type (see ref/TIERS.md). `satisfies Record<Block["type"], BlockTier>` means a
 ...
 * `OpinionBlockType` (the mechanical, schema-derived source of truth built in
 ...
 * against the *type*, not a runtime check against the value (`OPINION_BLOCK_TYPES`/
 * `isOpinionBlockType`, also exported by that package) — a real Turbopack interop bug misresolves
 ...
 */
import type { Block, OpinionBlockType } from "@rebar-ui/placement";
import type { BlockTier } from "./tier.types";
...
// @rebar-ui/placement's real OpinionBlockType members, and every real OpinionBlockType member must
...
type _NoExtraOpinionsHere = Exclude<OpinionEntriesHere, OpinionBlockType> extends never ? true : never;
type _NoMissingOpinionsHere = Exclude<OpinionBlockType, OpinionEntriesHere> extends never ? true : never;
